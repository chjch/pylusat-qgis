...
class SelectByLocation(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        within_dist.setMetadata({"widget_wrapper": {"decimals": 3}})
        self.addParameter(within_dist)
        self.addParameter(
            QgsProcessingParameterVectorDestination(
                self.OUTPUT, self.tr("Selected")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        select_lyr = self.parameterAsVectorLayer(
            parameters, self.SELECT, context
        )
        op = self.op_option[
            self.parameterAsEnum(parameters, self.OP, context)
        ][0].lower()
        ...
        select_gdf = PyLUSATQUtils.vector_to_gdf(select_lyr)
        output = geotools.select_by_location(
            input_gdf, select_gdf, op, within_dist
        )
        if not output.empty:
            ...
            feedback.pushInfo(
                "No features in input layer met the specified "
